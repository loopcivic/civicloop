...
// // ✅ NEW: Auto-zoom logic to fit all pins perfectly
// function InitialFitBounds({ complaints }: { complaints: any[] }) {
//   const map = useMap();
//   const [hasFitted, setHasFitted] = useState(false);

//   useEffect(() => {
//     // Prevent interrupting the user if we've already zoomed once, or if data is empty
//     if (hasFitted || !complaints || complaints.length === 0) return;

//     // Mathematically find the edges of all complaints
//     const bounds = L.latLngBounds(complaints.map(c => [c.lat, c.lng]));

//     if (bounds.isValid()) {
//       map.fitBounds(bounds, {
//         padding: [50, 50], // Leaves a nice margin around the edges on desktop/mobile
//         maxZoom: 14,       // Prevents zooming in uncomfortably close if there is only 1 pin
//         animate: true,
//         duration: 1.5      // Smooth cinematic zoom
//       });
//       setHasFitted(true);
//     }
//   }, [map, complaints, hasFitted]);
...
//   return (
//     <MapContainer
//       // ✅ CHANGED: Default coordinates set to the geographical center of India
//       center={[20.5937, 78.9629]}
//       zoom={5} // Zoomed out enough to see the whole country initially
//       scrollWheelZoom={true}
//       className="h-full w-full outline-none z-0"
//       style={{ background: "#09090b" }}
//     >
...
//       {/* ✅ NEW: Placed the Fit Bounds Handler here */}
//       <InitialFitBounds complaints={complaints} />
...
// ✅ UPDATED: Vibrant Pink User Location Marker (Fixed Comments)
const userLocationIcon = L.divIcon({
  ...
  html: `
    <div class="relative flex h-6 w-6 justify-center items-center">
      ...
      <span class="relative inline-flex rounded-full h-4 w-4 bg-pink-500 border-2 border-white shadow-[0_0_12px_rgba(236,72,153,1)]"></span>
    </div>
  `,
  iconSize: [24, 24],   
  iconAnchor: [12, 12], 
...
export function UserLocationMarker() {
  ...
  useEffect(() => {
    ...
    // Extract functions so we can properly remove them on cleanup
    const handleLocationFound = (e: L.LocationEvent) => {
      setPosition(e.latlng);
    };

    const handleLocationError = (e: L.ErrorEvent) => {
      console.warn("Could not get map location:", e.message);
    };

    map.on("locationfound", handleLocationFound);
    map.on("locationerror", handleLocationError);

    // ✅ FIX: Clean up event listeners when the component unmounts!
    return () => {
  ...
  return (
    <Marker 
      ...
      zIndexOffset={9999} // Forces the user dot to ALWAYS render above clusters
    >
      <Popup className="font-sans font-bold text-sm">
        <div className="text-zinc-900">📍 You are here</div>
      </Popup>
    </Marker>
  );
}
...
export default function Map({
  ...
  return (
    <MapContainer
      // ✅ CHANGED: Default coordinates set to Varanasi, Uttar Pradesh
      center={[25.3176, 82.9739]}
      zoom={12} // Zoomed in closer to the city level
      scrollWheelZoom={true}
      className="h-full w-full outline-none z-0"
      style={{ background: "#09090b" }}
    >
      ...
      {/* ✅ NEW: Drop the live tracking dot here! */}
      <UserLocationMarker />
