...
export default function AdminDashboard() {
  ...
  async function loadDashboardData() {
    try {
      setLoading(true);
      const [complaintsData, wardsData, deptsData, staffData, metricsData] = await Promise.all([
        apiCall("/complaints").catch(() => []),
        apiCall("/meta/wards").catch(() => []),
        apiCall("/meta/departments").catch(() => []),
        apiCall("/auth/users").catch(() => []),
        apiCall("/metrics/dashboard").catch(() => ({ resolutionTrends: [], slaData: [], topOfficers: [] }))
      ]);
        ...
        <div className="p-4 sm:p-8 w-full max-w-[1400px] mx-auto">
          ...
          {activeTab === 'OVERVIEW' && (
            <div className="space-y-6 sm:space-y-8">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 bg-zinc-900/50 border border-zinc-800 rounded-2xl p-4 sm:p-6">
                  <h3 className="text-base sm:text-lg font-bold text-white mb-4 flex items-center gap-2">
                    <TrendingUp className="text-pink-500 w-5 h-5" /> 7-Day Resolution Trend
                  </h3>
                  <div className="h-[250px] w-full text-xs">
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart data={metrics.resolutionTrends} margin={{ top: 5, right: 20, bottom: 5, left: -20 }}>
                        ...
                        <Line type="monotone" name="Reported" dataKey="reported" stroke="#f43f5e" strokeWidth={3} dot={{ r: 4, strokeWidth: 2 }} activeDot={{ r: 6 }} />
                ...
                <div className="bg-zinc-900/50 border border-zinc-800 rounded-2xl p-4 sm:p-6">
                  <h3 className="text-base sm:text-lg font-bold text-white mb-4 flex items-center gap-2">
                    <AlertCircle className="text-blue-500 w-5 h-5" /> SLA Compliance (48h)
                  </h3>
                  <div className="h-[250px] w-full text-xs">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={metrics.slaData} layout="vertical" margin={{ top: 5, right: 5, bottom: 5, left: 0 }}>
                        <CartesianGrid strokeDasharray="3 3" stroke="#27272a" horizontal={false} />
                        <XAxis type="number" stroke="#71717a" hide />
                        <YAxis dataKey="department" type="category" stroke="#71717a" width={80} tick={{fill: '#a1a1aa', fontSize: 11}} tickLine={false} axisLine={false} />
                        <Tooltip 
                          cursor={{fill: '#27272a'}}
                          contentStyle={{ backgroundColor: '#18181b', borderColor: '#27272a', borderRadius: '8px' }}
                        />
                        <Legend />
                        <Bar dataKey="onTime" name="% On Time" stackId="a" fill="#3b82f6" radius={[0, 0, 0, 0]} />
                        <Bar dataKey="breached" name="% Breached" stackId="a" fill="#3f3f46" radius={[0, 4, 4, 0]} />
                      </BarChart>
                    </ResponsiveContainer>
                  </div>
                </div>
              ...
              {/* Bottom Data Grid: Leaders & Recent Activity */}
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                
                {/* Top Performing Officers */}
                <div className="bg-zinc-900/50 border border-zinc-800 rounded-2xl p-4 sm:p-6">
                  <h3 className="text-base sm:text-lg font-bold text-white mb-4 flex items-center gap-2">
                    <Award className="text-yellow-500 w-5 h-5" /> Top Responders
                  </h3>
                  <div className="space-y-3">
                    {metrics.topOfficers.length === 0 ? (
                      <p className="text-zinc-500 text-sm">Not enough data to rank officers yet.</p>
                    ) : (
                      metrics.topOfficers.map((off: any, idx: number) => (
                        <div key={off.id} className="flex items-center justify-between p-3 bg-black/40 rounded-xl border border-zinc-800/50">
                          <div className="flex items-center gap-3">
                            <div className="w-8 h-8 rounded-full bg-yellow-500/10 text-yellow-500 flex items-center justify-center font-bold text-xs border border-yellow-500/20 shrink-0">
                              #{idx + 1}
                            </div>
                            <div className="overflow-hidden">
                              <div className="text-sm font-bold text-white truncate">{off.name}</div>
                              <div className="text-[10px] text-zinc-500 uppercase">{off.role}</div>
                            </div>
                          </div>
                          <div className="text-right shrink-0 ml-2">
                            <div className="text-sm font-bold text-green-400">{off.cases}</div>
                ...
                {/* Recent Activity Table */}
                <div className="lg:col-span-2 bg-zinc-900/50 border border-zinc-800 rounded-2xl p-4 sm:p-6 overflow-hidden flex flex-col min-h-[300px]">
                  <h3 className="text-base sm:text-lg font-bold text-white mb-4">Recent Critical Alerts</h3>
                  <div className="space-y-2 sm:space-y-3 flex-1 overflow-y-auto pr-2 custom-scrollbar">
                    {complaints.length === 0 ? (
                      <p className="text-zinc-500 text-sm">No recent alerts found.</p>
                    ) : (
                      complaints.slice(0, 5).map(c => (
                        <div key={c.id} className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 p-3 bg-black/20 hover:bg-zinc-800 rounded-lg border border-zinc-800/50 transition">
                          <div className="flex items-start sm:items-center gap-3">
                          ...
                          <span className={`text-[10px] sm:text-xs font-bold px-2 py-1 rounded self-end sm:self-auto shrink-0 ${c.currentStatus === 'RESOLVED' ? 'bg-green-900/30 text-green-400' : 'bg-zinc-800 text-zinc-400'}`}>
                            {c.currentStatus}
                          </span>
                        </div>
                      ))
